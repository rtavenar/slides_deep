...
#show: university-theme.with(
  aspect-ratio: "4-3",
  align: horizon,
  footer-b: [Deep Learning - Loss functions, optimization, regularization],
  config-info(
    title: [Deep Learning],
    subtitle: [3. Loss functions, optimization, regularization],
...
- In practice, *scaling input features* is critical for stable training
  ...
  - Recommendation: center-reduce each feature (zero mean, unit variance)

$
  tilde(x)_j = (x_j - mu_j) / sigma_j
$

#image-with-caption(image("fig/standardization.svg", width: 70%), [_Standardized inputs → faster, more stable convergence_])

- In `keras`, preprocessing can be done before fitting or via a `Normalization` layer

== Loss functions

- To train a model, we need to *measure how wrong it is* — that's the loss function

- Standard losses
  - *Mean Squared Error (MSE)* for regression — penalizes large errors quadratically
    $ cal(L)(x_i, y_i; theta) = (m(x_i; theta) - y_i)^2 $
  - *Cross-entropy* for classification — penalizes confident wrong predictions
...
- Technical requirement for gradient descent: $cal(L)$ must be differentiable, which feeds into backpropagation:

$
  (partial cal(L))/(partial w^((0))) = (partial cal(L))/(partial a^((3))) (partial a^((3)))/(partial o^((3))) (partial o^((3)))/(partial a^((2))) (partial a^((2)))/(partial o^((2))) (partial o^((2)))/(partial a^((1))) (partial a^((1)))/(partial o^((1))) (partial o^((1)))/(partial w^((0)))
$
...
- *Intuition:* keep signal "loudness" (variance) *constant* across layers
  ...
  - too small → fade to 0 (vanishing gradients)
...
- *Xavier / Glorot* (tanh, sigmoid):
  ...
  - stable forward ($n_"in"$) *and* backward ($n_"out"$) → *average*
...
- *init fixes scale only at $t=0$* → drifts as weights update → BatchNorm #sym.arrow.r

== Keeping activations well-scaled #linebreak() Batch Normalization

#grid(
  columns: (1fr, 1fr),
  gutter: 1em,
  [
    - Same goal as init, *enforced during training*: re-standardize layer inputs *every* mini-batch
      $ hat(z)^((l)) = (z^((l)) - mu_cal(B)) / sqrt(sigma_cal(B)^2 + epsilon) $
      - then learnable scale $gamma$ + shift $beta$

    - *One family, 3 moments:*
      - input standardization
      - init → scale at start
      - BatchNorm → scale during training

    - Benefits: less covariate shift, higher LR, mild regularizer
  ...
  [
    #image-with-caption(image("fig/batchnorm.svg", width: 100%), [_BatchNorm → faster, more stable training_])
  ]
)
...
- Neural networks are usually used in over-parametrized mode
  - Need regularization: early stopping, L2, Dropout
  - Need to check performance on hold-out data (validation set)
- Keep activations well-scaled (same idea, 3 moments): input standardization, init, BatchNorm
